...
exports.create = async (req, res) => {
  try {
    ...
    return res.status(500).send(
      errorResponse({
        status: 500,
        message: "Some error occured while creating activity log",
      })
    );
  }
};
...
exports.findOne = async (req, res) => {
  try {
    ...
    return res.status(500).send(
      errorResponse({
        status: 500,
        message: "Failed to fetch the activity log",
      })
    );
  }
};
...
exports.update = async (req, res) => {
  try {
    const sequelize = req.db;
    const id = req.params.id;
    const [updatedRowsCount, updatedLog] = await sequelize.models.Activity_log.update(req.body, {
      where: { id: id },
      returning: true,
    });
    if (updatedRowsCount === 0) {
      return res.status(404).send(
        errorResponse({
          ...
          details: "Invalid id to fetch activity log",
        })
      );
    }
    return res.status(200).send({
...
exports.delete = async (req, res) => {
  try {
    ...
    if (deletedRowCount === 0) {
      return res.status(404).send(
        errorResponse({
          ...
          details: "activity log id is invalid",
        })
      );
    }
    ...
    return res.status(500).send(
      errorResponse({
        status: 500,
        message: "Failed to delete the activity log",
      })
    );
  }
};
